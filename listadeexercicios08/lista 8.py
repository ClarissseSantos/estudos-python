...
#NOME2:Douglas Henrique Teixeira Barboza
#Turma A - 1ºADS
####
...
for p in txtB:
    if p[0] in zumbi and p[-1] not in zumbi:
        cont += 1
print('Existem',cont,'preposições')
#Questão B
print('Questão B')
cont = 0
cont2 = 0
for p in txtB:
    if len(p) == 7  and p[-1] not in zumbi:
        cont = cont + 1
        if p[0] not in zumbi:
            cont2 = cont2 + 1
print('São',cont,'verbos')
print('Questão C')
print('Estão em 1ª pessoa',cont2,'verbos')
#Questão D
print ('Questão D')
ordem = 'zmbtshjpnwlrcxkqvdgf'
txtB.sort()
for c in ordem:
     chr(65+ ordem.index(c))
def converte(s):
    resp = ''
    for c in s :
        resp = resp + chr(65 + ordem.index(c))
    return resp
newB = [converte(p) for p in txtB]
newB.sort()
newB
ordem
ordem[ord('B') - 65]
